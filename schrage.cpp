...
///	cout << "\n\n";
	...
	while (wektorG.empty() == false || wektorN.empty() == false) { ///dopoki wektor G lub wektor N nie sa puste
		...
		while (wektorN.empty() == false && wektorN.top().r <= t) { ///dopoki wektor N nie jest pusty oraz czas przygotowania pierwszego zadania jest mniejszy badz rowny czasowi t
